...
class DebuggerUtility
{
    ...
    protected static function renderObject($object, $level, $plainText = false, $ansiColors = false)
    {
        ...
        if ($level < self::$maxDepth && !self::isBlacklisted($object) && !(self::isAlreadyRendered($object) && true !== $plainText)) {
        ...
        if ($object instanceof \Closure) {
            ...
            foreach ($reflectionFunction->getParameters() as $parameter) {
                ...
                if ($parameter->isDefaultValueAvailable()) {
                    $parameterDump .= ' = ';
                    if ($plainText) {
                        $parameterDump .= self::ansiEscapeWrap(var_export($parameter->getDefaultValue(), true),
                            '33', $ansiColors);
                    } else {
